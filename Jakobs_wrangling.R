...
id_comp_new <- id_comp_new %>% distinct(id, New_name, .keep_all = TRUE)
...
#Don't use the accumulated MW with this sheet

product <- df %>%
  select(id, project_name,electrolyser_mw, product,production_ammount, end_use_mobility, end_use_heat_power, end_use_grid_export) %>%
  separate_rows(product,production_ammount, sep= ", ")
write_sheet(product,ss,sheet="products")

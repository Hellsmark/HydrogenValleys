...
box<- sel %>% 
  ggplot(aes(x = country, y = electrolyser_mw,
             label = project_name,
             fill = country)) +
  geom_boxplot() +
...
PROBLEM: Nel has projects in Denmark, Norway and Sweden, but it doesn't show
...
data_summary <- actors %>%
  count(comp_name, country) %>%
  arrange(desc(n)) %>%
  head(30)

ggplot(data_summary, aes(x = comp_name, y = n, fill = country)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(
    title = "Most represented actors",
    x = "Companies",
    y = "Number of projects",
    fill = "Country"
  ) +
  ...
  theme(
  axis.text.x = element_text(angle = 45, hjust = 1)  # Snedställda etiketter
  )
ggsave("figs/Most_represented_actors.png")
```
...
PROBLEM: Projects with multiple products should not be presented like this
...
ggsave("figs/Network_Product_companies.png")
```

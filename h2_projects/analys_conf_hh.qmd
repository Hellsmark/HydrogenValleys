...
options: 
 - map
 - H2 over time
 - Illustrate actors across - countries, see who connects across multiple projects
 - illustrate correlation between variables? Here one could also do a cluster analysis - later
...
# H2 over time - Done
Illustrates what year the projects are planned to be operational in different countries
- For this figure mention main project
- The number of projects per country
- And outline main differences.
...
## Table
```{r}
tot <- sum(over_time$mw)
over_time %>% group_by(status) %>% 
  summarise(mw = sum(mw), share = round(mw/tot, 2), TWH = round(mw*8000/1e6)) %>% arrange(TWH) %>% gt::gt() %>% gt_theme_espn() %>% gt::gtsave("h2_projects/figs_conf/1_table.png")

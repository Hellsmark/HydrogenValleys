...
# The following code will tell us how many observations per group that we need in order to get a statistically significant result. (k is number of gruops that we study) This is not a 100 percent accurate since we dont do ANOVA
pwr_result <- pwr.anova.test(k = 2, f = 0.25, sig.level = 0.05, power = 0.8)
pwr_result
...
# Try different org types here, for example job role or sector
...
ggplot(mark, aes(x=org_type, y=counter)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1))
```
...etc etc for all barriers



The following code makes a friedman test on the full data of the ranking to see if they are made randomly or if there is a significant difference in how people vote. 

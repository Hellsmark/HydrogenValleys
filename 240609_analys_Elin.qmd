...
hyd_spec_no <- company_types %>% filter(Hydrogen_specialist == "Yes") %>% filter(Country == "NO") 
not_spec_no <- company_types %>% filter(Hydrogen_specialist == "No") %>% filter(Country == "NO")

hyd_spec_dk <- company_types %>% filter(Hydrogen_specialist == "Yes") %>% filter(Country == "DK") 
not_spec_dk <- company_types %>% filter(Hydrogen_specialist == "No") %>% filter(Country == "DK")

hyd_spec_se <- company_types %>% filter(Hydrogen_specialist == "Yes") %>% filter(Country == "SE") 
not_spec_se <- company_types %>% filter(Hydrogen_specialist == "No") %>% filter(Country == "SE")
...
gt_tbl <- tbl %>%
  tbl_cross(Sector, Country, percent = "column") %>%
  as_gt() %>%
  gt_theme_espn() 
...
```{r}
sectors_norm <- tbl %>% group_by(Sector, Country) %>%
...
gt_tbl <- tbl %>% rename("Type of job" = job_role) %>%
  tbl_cross("Type of job", Country, percent = "column") %>%
  as_gt() %>%
  gt_theme_espn() 
...
```{r}
mean_per_jobrole <- jobs_norm %>%
  group_by(job_role) %>%
  summarise(mean_n_job = mean(n_norm))

jobs_norm <- merge(jobs_norm, mean_per_jobrole, by = "job_role", suffixes = c("", "_jobrole"))

jobs_norm$diff_n_job <- jobs_norm$n_norm - jobs_norm$mean_n_job

jobroles_deviation_plot_2 <- ggplot(jobs_norm, aes(x = str_wrap(job_role, 20, whitespace_only = FALSE), y = diff_n_job*100)) +
  geom_col(aes(fill = Country), width = 0.5, position = position_dodge(width = 0.5)) +
  scale_fill_colorblind("Country") +
  theme(legend.position = "top") +
  labs(y = "Deviation from mean", x = "") +
  theme_bw() +
  theme(axis.text.x = element_text(size = 8, angle = 45, hjust = 1, vjust = 1)) 
jobroles_deviation_plot_2

sd_jobrole <- jobs_norm %>%
  group_by(job_role) %>%
  summarise(sd_n_job = sd(n_norm))

jobs_norm <- merge(jobs_norm, sd_jobrole, by = "job_role", suffixes = c("", "_jobrole"))

diff_significant <- jobs_norm %>% filter(diff_n_job < (-1)*sd_n_job | diff_n_job > sd_n_job)
```
...
tbl %>% filter(job_role == "Engineering") %>%
...
#ggsave("figs/Total_jobs_per_engineering_type.png", dpi = 1200, width = 6, height = 6)
...
engineers_deviation_plot <- ggplot(engineers_norm, aes(x = str_wrap(Type_2, 20, whitespace_only = FALSE), y = diff*100)) +
  geom_col(aes(fill = Country), width = 0.5, position = position_dodge(width = 0.5)) +
  scale_fill_colorblind("Country") +
  theme(legend.position = "top") +
  labs(y = "Deviation from mean", x = "") +

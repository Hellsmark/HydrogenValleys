...
df_distribution <- jobs %>% 
  filter(country_code %in% c("SE","NO","DK")) %>%
  count(Company, country_code) %>% 
  filter(!Company == "!!!NEW_COMPANY!!!")

violine1 <- df_distribution %>%
...
violine2 <- df_distribution %>%
  rename(Country = country_code) %>%
  ggplot(aes(Country, n, fill = Country), alpha = 0.5) + 
  geom_violin() + 
  scale_fill_colorblind() + 
  theme_grey() +
...
Tried adding violine together with table
...
final_figure1 <- cowplot::plot_grid(violine1,
                                                    table_to_plot,
                                                    nrow = 1,
                                                    ncol = 2,
                                                    rel_heights = c(1, 1))
...
final_figure2 <- cowplot::plot_grid(violine2,
                                                    table_to_plot,
                                                    nrow = 1,
                                                    ncol = 2,
                                                    rel_heights = c(1, 1))

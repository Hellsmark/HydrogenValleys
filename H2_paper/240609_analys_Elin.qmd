...
comp <- jobs %>% # Number på companies in advertising in each country (not country of joblocation)
...
top10_companies_perCountry <- jobs %>%
  ...
  slice_head(n = 10) %>%
  ungroup() %>%
  filter(country %in% c("Norway","Denmark","Sweden")) %>%
  select(country,Company,n)
...
```{r}
top10_companies_perCountry

top10_new <- function(cc){
  ...
  select(-country) %>%
  gt(rowname_col = "Company") %>%
  tab_stubhead(label = "Organisations") %>%
  grand_summary_rows(
    columns = c(n, share, share_tot),
    fns = list(
      Total = ~ sum(.)
...
dk_tbl %>% gtsave("figs/cluster_paper_plots/dk_tbl_top10_companies.png")
no_tbl %>% gtsave("figs/cluster_paper_plots/no_tbl_top10_companies.png")
se_tbl %>% gtsave("figs/cluster_paper_plots/se_tbl_top10_companies.png")
...
types <- companies %>% 
 ...
 # filter(location_2 %in% c("DK","NO","SE", "DK ","NO ","SE ", " DK"," NO"," SE") | is.na(location_2)) #  mutate(multi_Nordic = ifelse(str_length(Location_of_jobs)>2, TRUE, FALSE))

company_types <- jobs %>% 
  filter(nation %in% c("DK","NO","SE")) %>%
  group_by(Company, nation) %>% 
  summarise(n = n()) %>%
  left_join(types, by = c("Company" = "Name")) %>%
  select(Company, nation, n, Multinational, Organisation_type, Ownership_type,
...
df_gs <- jobs %>% 
    filter(nation %in% c("DK","NO","SE")) %>%
  count(Company, nation, name = "weight") %>% 
  filter(!Company == "!!!NEW_COMPANY!!!") %>%
  left_join(types, by =c("Company" = "Name")) %>% 
  na.omit() %>% 
...
```{r}
sectors_norm <- tbl %>% 
  group_by(Sector, nation) %>%
  summarise(n_job = n(), .groups = 'drop') %>%
  complete(Sector, nation, fill = list(n_job = 0)) 

jobs_total <- aggregate(n_job ~ nation, data = sectors_norm, sum)

sector_total <- aggregate(n_job ~ Sector, data = sectors_norm, sum)

sectors_norm <- merge(sectors_norm, jobs_total, by = "nation", suffixes = c("", "_country"))
...
sector_deviation_plot <- ggplot(sectors_norm, aes(x = str_wrap(Sector, 20, whitespace_only = FALSE), y = diff*100)) +
  geom_col(aes(fill = nation), width = 0.5, position = position_dodge(width = 0.5)) +
  scale_fill_colorblind("Country") +
...
```{r}
tbl$Trading <- ifelse(tbl$Sector %in% c("Energy Utility", "Oil/gas/refinery", "Chemicals", "Iron/steel/mining", "Transport"), "Trading", "Non-trading")

trading <- tbl %>% filter(Trading == "Trading")
nontrading <- tbl %>% filter(Trading == "Non-trading")

trading_or_not <- tbl %>% 
  group_by(nation, Sector, Trading) %>%
...
trading_nontrading_summary <- tbl %>%
  mutate(Trading = ifelse(Sector %in% c("Energy Utility", "Oil/gas/refinery", "Chemicals", "Iron/steel/mining", "Transport"), "Trading", "Non-trading")) %>%
  group_by(nation, Sector, Trading) %>%
  summarise(n = n(), .groups = 'drop') %>%
...
trading_nontrading_plot <- ggplot(trading_nontrading_summary, aes(x = x_value, y = fct_rev(Sector), fill = nation)) + 
  geom_col() +
  scale_x_continuous(labels = abs, limits = c(-max_x_value, max_x_value), expand = c(0, 0)) + # Set x-axis limits to center y-axis
  scale_fill_colorblind("Country") +
...
jobs_norm <- tbl %>% group_by(job_role, nation) %>%
  summarise(n_job = n(), .groups = 'drop') %>%
  complete(job_role, nation, fill = list(n_job = 0)) 

jobs_total <- aggregate(n_job ~ nation, data = jobs_norm, sum)

jobrole_total <- aggregate(n_job ~ job_role, data = jobs_norm, sum)

jobs_norm <- merge(jobs_norm, jobs_total, by = "nation", suffixes = c("", "_country"))
jobs_norm <- merge(jobs_norm, jobrole_total, by = "job_role", suffixes = c("", "_jobrole"))

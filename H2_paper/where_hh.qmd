...
outlier_tbl <- function(df,std_size){
  ...
  gt() %>%
  tab_header(
    title = paste0("Regions diverting ",as.character(std_size)," standard deviation",ifelse(std_size==1,"","s")," from mdoel")
  ) %>%
  cols_label(
    region = "Region",
    country = "Country",
...
all_regions <- all_regions %>%
  mutate(
    region_type = ifelse(name %in% highlight_regions, paste(country, "Highlighted"), country)
  )

# Define color mapping for both countries and highlighted regions
color_mapping <- c(
  "Sweden" = "lightblue",
  "Norway" = "lightgreen",
  "Denmark" = "lightpink",
  "Sweden Highlighted" = "orange",
  "Norway Highlighted" = "orange",
  "Denmark Highlighted" = "orange"
)

# Plot the combined map with specific regions highlighted
combined_map <- ggplot() +
  #annotation_map_tile(type = "cartolight", zoomin = 0) +
  geom_sf(data = all_regions, aes(fill = region_type), colour = "grey40", linewidth = 0.5) +

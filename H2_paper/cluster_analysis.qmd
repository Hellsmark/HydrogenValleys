...
```{r}
# I will have to do this seperately for sectors and then for the jobroles
cluster_tbl_data <- jobs_cluster_nicename %>%
  select(Group = cluster, Region = place, Sectors = sector, Job_Roles = competence) %>%
  unique() %>%
...
```{r}
cluster_tbl <- jobs_cluster_nicename %>%
  gt(rowname_col = "")
...
```{r}
data <- tibble(
  Groups = c("Nordic", "Nordic", "Nordic", "Middle", "Southern", "Southern"),
  Region = c("Boden", "Luleå", "Umeå", "Stockholm", "Göteborg", "Malmö"),
  Sectors = c("Finance, Energy", "Finance, Steel", "Finance, Energy, Steel", "Government", "Research", "Research, IT")
)

# Function to add border conditionally
add_border <- function(data, group_col, group_val) {
  data %>%
    gt() %>%
    tab_header(
    ...
    tab_style(
      ...
      locations = cells_body(
        rows = data[[group_col]] == group_val
      )
    )
}

# Apply the function to the data
table <- add_border(data, "Groups", "Nordic")
...
data %>% 
  ...
  tab_header(
    title = md("The groups"),
    subtitle = md("Sectors listed to their presence in corresponding region in group")
  )

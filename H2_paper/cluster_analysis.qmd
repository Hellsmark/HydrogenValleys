...
jobs_cluster_nicename <- jobs_clustered %>%
  mutate(cluster = case_when(
    ...
    cluster == 2 ~ "Others",
    cluster == 3 ~ "Others",
    cluster == 4 ~ "Central offices in Oslo/Copenhagen",
    cluster == 5 ~ "Danish and Norwegian Research",
...
cluster_tbl_data_short <- merge(
  cluster_tbl_data %>% select(Group, Regions = Region) %>% unique() %>%
    pivot_wider(names_from = "Regions", 
                values_from = Regions) %>%
    select(Group, "Agder (NO)" = "Agder, NO", "Dalarna (SE)" = "Dalarna, SE",
           "Gotland (SE)" = "Gotland, SE", "Gävleborg (SE)" = "Gävleborg, SE",
           "Halland (SE)" = "Halland, SE", "Hovedstaden (DK)" = "Hovedstaden, DK", 
           "Jämtland (SE)" = "Jämtland, SE", "Kalmar (SE)" = "Kalmar, SE", 
           "Kronoberg (SE)" = "Kronoberg, SE", "Midtjylland (DK)" = "Midtjylland, DK", 
           "Møre og Romsdal (NO)" = "Møre og Romsdal, NO", 
           "Nordjylland (DK)" = "Nordjylland, DK", "Nordland (NO)" = "Nordland, NO",
           "Norrbotten (SE)" = "Norrbotten, SE", "Oslo (NO)" = "Oslo, NO", 
           "Rogaland (NO)" = "Rogaland, NO", "Sjælland (DK)" = "Sjælland, DK", 
           "Skåne (SE)" = "Skåne, SE", "Stockholm (SE)" = "Stockholm, SE", 
           "Syddanmark (DK)" = "Syddanmark, DK", "Södermanland (SE)" = "Södermanland, SE",
           "Troms og Finnmark (NO)" = "Troms og Finnmark, NO", 
           "Trøndelag (NO)" = "Trøndelag, NO", "Uppsala (SE)" = "Uppsala, SE", 
           "Vestfold og Telemark (NO)" = "Vestfold og Telemark, NO", 
           "Vestland (NO)" = "Vestland, NO", "Viken (NO)" = "Viken, NO", 
           "Värmland (SE)" = "Värmland, SE", "Västerbotten (SE)" = "Västerbotten, SE",
           "Västernorrland (SE)" = "Västernorrland, SE", 
           "Västmanland (SE)" = "Västmanland, SE", 
           "Västra Götaland (SE)" = "Västra Götaland, SE", 
           "Östergötland (SE)" = "Östergötland, SE") %>%
...
Most important thing for each region
...
# Any sector/region/company/competence which only holds 1 job?

```{r}
only1_sector <- jobs_cluster_nicename %>%
  group_by(sector) %>%
  summarise(n = n()) %>% 
  ungroup()
  

only1_region <- jobs_cluster_nicename %>%
  group_by(place) %>%
  summarise(n = n()) %>% 
  ungroup()

only1_company <- jobs_cluster_nicename %>%
  group_by(company) %>%
  summarise(n = n()) %>% 
  ungroup()

only1_competence <- jobs_cluster_nicename %>%
  group_by(competence) %>%
  summarise(n = n()) %>% 
  ungroup()

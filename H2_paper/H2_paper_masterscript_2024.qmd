...
gc_jobs %>% ggraph(layout = "nicely") + 
  ...
  labs(title = "Actors Cutting Across the Nordic countries", 
       caption = "Most central actors cutting across multiple countries. Each actor have announced a total of jobs equal to or more than 1 jobs")+
  theme_graph(background = background) + 
  theme(legend.position = 'none')

ggsave("H2_paper/figs_W2425/actors_across_countries.png", width = 13.33, height = 7.5, units = "in", dpi = 300)
```
...
regionsss <- geo %>% 
  ...
  unique() %>%
  filter(country_code %in% c('SE','DK','NO')) %>%
  filter(!region == 'Greenland')
```
...
map_norwegian_engineering <- ggplot() +
  annotation_map_tile(type = "cartolight", zoomin = 0) +
  geom_sf(data = regions_norwegian_engineering, aes(fill = cluster), colour = "grey40", linewidth = 0.5) +

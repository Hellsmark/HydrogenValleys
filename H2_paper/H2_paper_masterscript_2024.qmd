...
## Run multiple seeds for robustness 1

```{r}
set.seed(123)  # Set an initial seed for reproducibility
num_runs <- 500  # Number of times to run k-means
k <- 6  # Set number of clusters (adjust as needed)
cluster_results <- list()  # Empty list to store results
seeds_used <- list()

# Run k-means multiple times with different seeds
for (i in 1:num_runs) {
  seed_current <- sample(1:10000, 1) # Randomize seed for each run
  set.seed(seed_current) 
  km <- kmeans(mtrx_factor_count, centers = k, iter.max = 40, nstart = 50)
  cluster_results[[i]] <- km$cluster  # Store cluster assignments
  seeds_used[[i]] <- seed_current
}

# Convert list to a dataframe for comparison
df_clusters <- as.data.frame(do.call(cbind, cluster_results))
...
most_common_count <- max(cluster_counts$Freq)
most_common_config <- cluster_counts$cluster_patterns[1]

print(paste("Most common cluster configuration occurred", most_common_count, "times out of", num_runs))
print(paste("Most common configuration:", most_common_config))
...
# Compute mean ARI across all pairs
mean_ari <- mean(ari_values)

print(paste("Mean Adjusted Rand Index across runs:", round(mean_ari, 4)))
```

```{r}
library(pheatmap)

# Create a matrix of cluster assignments (rows = data points, columns = runs)
cluster_matrix <- as.matrix(df_clusters[, 1:num_runs])

# Create a heatmap where colors represent different clusters
# Optionally, you can use a color palette to distinguish the clusters
pheatmap(cluster_matrix, cluster_rows = FALSE, cluster_cols = FALSE, 
         color = colorRampPalette(c("white", "steelblue", "red"))(k), 
         main = "Cluster Assignments Across Runs")

###

# Create a comparison matrix: each cell (i,j) represents how often run i and run j agree
comparison_matrix <- matrix(0, nrow = num_runs, ncol = num_runs)

# Compare each pair of runs
for (i in 1:num_runs) {
  for (j in 1:num_runs) {
    comparison_matrix[i, j] <- sum(df_clusters[[i]] == df_clusters[[j]]) / nrow(df_clusters)
  }
}

# Visualize comparison matrix as a heatmap
pheatmap(comparison_matrix, cluster_rows = FALSE, cluster_cols = FALSE, 
         main = "Pairwise Comparison of Clustering Across Runs")

###

# Calculate how often each data point is assigned to each cluster across runs
cluster_freq <- apply(df_clusters[, 1:num_runs], 1, table)  # Table of frequencies for each data point

# Convert to data frame for ggplot
df_freq <- as.data.frame(do.call(rbind, cluster_freq))
colnames(df_freq) <- paste0("Cluster_", 1:k)  # Name columns as Cluster_1, Cluster_2, etc.
df_freq$Point <- 1:nrow(df_freq)

# Reshape for ggplot
df_freq_long <- gather(df_freq, key = "Cluster", value = "Frequency", -Point)

# Plot
ggplot(df_freq_long, aes(x = Point, y = Frequency, fill = Cluster)) + 
  geom_bar(stat = "identity", position = "stack") +
  theme_minimal() +
  labs(title = "Frequency of Cluster Assignments Across Runs",
       x = "Data Point", y = "Frequency of Cluster Assignment")

###

library(mclust)

# Example: Compute the Adjusted Rand Index (ARI) for two runs
ari_value <- adjustedRandIndex(df_clusters[[1]], df_clusters[[2]])

# Print ARI value
print(paste("Adjusted Rand Index between Run 1 and Run 2: ", round(ari_value, 4)))
```
...
# Step 7: Keep only first occurrence of each unique pattern
  ...
  print(paste0('k = ',as.character(j),' done!'))
    ...
    # LATER STEP IS TO EVALUATE THE RESULTS FROM THIS AND THEN MAKE A SELECTION OF SEEDS/CLUSTER CONFIGURATIONS AND WITH THAT HAVE PLOTS MADE FOR THE SILHOUETTE SCORES AND THE WSS IN ORDER TO VISUALLY SEE HOW GOOD THEY ARE - ALSO MAKE REGULAR K-MEANS COLORFUL PLOTS IN ORDER TO VISUALISE RESULTS OF THIS - ALSO MAKE CLUSTER TABLES IN ORDER TO SEE HOW THESE CLUSTERS LOOK LIKE

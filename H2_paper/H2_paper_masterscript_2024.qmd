...
(jobs %>% group_by(country, Company) %>% summarise(n =n()) %>% 
         ungroup() %>% arrange(desc(n)) %>% head(30) %>% select(n) %>% sum())/(jobs %>% group_by(country, Company) %>% summarise(n =n()) %>% 
...
biggest_companies_plot <- biggest_companies %>%
  ...
  geom_col(drop = F)+
    scale_x_continuous()+
  scale_fill_colorblind("Country")+
    labs(x= "", y = "", title = "Top 30 most recruiting organisations, responsible for 65% of all jobs")+
    theme_minimal()+
    theme(
      legend.position = "bottom", 
      text = element_text(size = 14), 
...
region_pop_plot <- region_new_jobs %>% 
  ...
  annotate("text", x=900, y =250, label = r2_p_no)+
  annotate("text", x=1500, y =220, label = r2_p_dk)+
  annotate("text", x=2250, y =200, label = r2_p_all)+
  annotate("text", x=2250, y =75, label = r2_p_se)+
  theme_minimal() +
  theme(legend.position = "top")
region_pop_plot

ggsave(filename = "figs/all_region_pop.png", plot = region_pop_plot, dpi= 600, width = 8, height = 4)
```
...
png("figs/k-mean_7.png", width = 800, height = 600)  # Open a PNG device
autoplot(KM_7,mtrx,frame=TRUE)
dev.off()  # Close the device

png("figs/k-mean_8.png", width = 800, height = 600)  # Open a PNG device
autoplot(KM_8,mtrx,frame=TRUE)
dev.off()  # Close the device

png("figs/k-mean_9.png", width = 800, height = 600)  # Open a PNG device
...
cluster_function <- function(matrx,k){
  ...
  k_mean_plot
  ggsave(paste0("figs/clusters/k-mean_",as.character(k),".png"))
...
add_cluster_to_data_function <- function(KM){
  ...
  se_cl <- df_hcl_long %>% group_by(cl, sector) %>% summarise(n =n()) %>% arrange(desc(cl))
...
make_table_function <- function(jobs_clustered,k){
...
cluster_tbl_jobroles <- jobs_clustered %>%
  ...
  pivot_wider(names_from = "Job_roles",
              values_from = Job_roles) %>%
  unite("Job_Roles", "Administration","Engineering", "Environmental",
        "Finance & Business development","HR", "IT & Data science", "Law", "Management",
        "Other","PhD", "Procurement & Logistics", "Project managment", 
...
cluster_tbl_data_short <- merge(
  ...
  merge(
    ...
    pivot_wider(names_from = "Job_roles",
                values_from = Job_roles) %>%
    unite("Job_Roles", "Administration","Engineering", "Environmental",
        "Finance & Business development","HR", "IT & Data science", "Law", "Management", "Other","PhD", "Procurement & Logistics", "Project managment", "Public administration", "Quality assurance", "Researcher", "Sales & Customer service",  "Technician & Maintenance",
          sep = ", ", na.rm = TRUE),
    by = "Group", all.x = TRUE
  )
...
cluster_tbl_short_WITH_BOLD %>% gtsave(paste0("figs/clusters/cluster_tbl_WITH_BOLD_",as.character(k),".png"), vheight = 800, vwidth = 2000)
}
```
...
```{r}
all_cluster_functions_together <- function(k,matrx){
  KM <- cluster_function(matrx,k)
  cluster_result <- add_cluster_to_data_function(KM)
  make_table_function(cluster_result$jobs_clustered,k)
...
for (i in 11:12){
  cluster_result_i <- all_cluster_functions_together(i,mtrx)
}
```

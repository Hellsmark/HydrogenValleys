...
(jobs_cluster_nicename %>% group_by(nation, company) %>% summarise(n =n()) %>% 
         ungroup() %>% arrange(desc(n)) %>% head(20) %>% select(n) %>% sum())/(jobs_cluster_nicename %>% group_by(nation, company) %>% summarise(n =n()) %>% 
...
biggest_companies_plot <- biggest_companies %>%
  ggplot(aes(x= n, y= fct_reorder(company, n),
             fill = nation)) + 
  geom_col(drop = F)+
    scale_x_continuous()+
  scale_fill_colorblind("Country")+

...
dend_color <- color_branches(hcl_obj, k = 6)
...
df_h2 <- jobs %>% 
  left_join(companies, by =c("Company" = "Name")) %>%
  select(id = ID, country = country_code, region, sector= Industry_Sector, competence= job_role) %>%
  na.omit() %>%
  unite(place, c("region", "country"), sep = ", ")
...
mca_data <- as.data.frame(mca_result$ind$coord)
...
Here I investigated what clusters we would have with only nation-sector basis.
...
Here we both look at what companies are part of the sectors in given regions in the clusters - but also do a new clustering with region/company instead of region/sector
...
Jobs over time with clusters instead of nations - Using k = 6 on seed = 123456789

This is without taking the companies into account

```{r}
jobs_over_time_clusters <- jobs_cluster_nicename %>%
...
cluster_overtime_plot <- jobs_over_time_clusters %>% 
  ggplot(aes(ym, n, fill= cluster)) + 
  geom_col() + 
  scale_fill_colorblind("Groups")+
  theme_minimal() +
  theme(legend.position = "bottom") +
  labs(x= "", y= "Nr. of new jobs") #+
  #theme(axis.text.x = element_text(size = 8, angle = 45, hjust = 1, vjust = 1)) 

cluster_overtime_plot

#ggsave(filename = "figs/cluster_paper_plots/cluster_overtime_plot.png", plot = cluster_overtime_plot, limitsize = FALSE, width = 12, height = 7)

```

## Regions and sector plots

Region and sector plots

```{r}
sector_per_region_plot <- jobs_clustered %>%
  mutate(place = str_replace(place, "(.*), (..)", "\\2, \\1")) %>% # change order of nation and region to make plot more readable
  group_by(place,sector) %>%
  summarise(n =n()) %>%
  ggplot(aes(x= n, y= fct_rev(sector), 
             fill = place)) + 
    geom_col()+
    scale_x_continuous()+
    scale_fill_viridis_d("Region", option = "turbo")+
...
sector_per_region_plot

#ggsave(filename = "figs/cluster_paper_plots/sector_per_region.png", plot = sector_per_region_plot, limitsize = FALSE, width = 14, height = 7)

# Not sure how informative this is. So many regions, makes it difficult to see difference between the colors. 
```

Try to make more informative by splitting it between countries and let sectors be colors instead

```{r}
sector_per_region <- jobs_clustered %>%
  mutate(place = str_replace(place, "(.*), (..)", "\\2, \\1")) %>% # change order of nation and region to make plot more readable
  group_by(place,sector) %>%
  summarise(n =n()) %>%
  separate(place, into = c("country", "region"), sep = ", ")

sector_per_region_dk <- sector_per_region %>%
  filter(country == "DK") %>%
  rbind(data.frame(country = c("DK","DK"), region = c("Hovedstaden","Hovedstaden"), sector = c("Iron/steel/mining","Transport"), n = c(0,0)))
sector_per_region_no <- sector_per_region %>%
  filter(country == "NO")
sector_per_region_se <- sector_per_region %>%
  filter(country == "SE") %>%
  rbind(data.frame(country = c("SE","SE"), region = c("Dalarna","Dalarna"), sector = c("Chemicals","Oil/gas/refinery"), n = c(0,0)))

sector_per_region_plot_dk <- sector_per_region_dk %>%
  ggplot(aes(x= n, y= fct_rev(region), 
             fill = sector)) + 
    geom_col()+
    scale_x_continuous()+
    scale_fill_viridis_d("Sector", option = "turbo")+
...
#ggsave(filename = "figs/cluster_paper_plots/sector_per_region_plot_dk.png", plot = sector_per_region_plot_dk, limitsize = FALSE, width = 14, height = 7)


sector_per_region_plot_no <- sector_per_region_no %>%
  ggplot(aes(x= n, y= fct_rev(region), 
             fill = sector)) + 
    geom_col()+
    scale_x_continuous()+
...
sector_per_region_plot_se <- sector_per_region_se %>%
  ggplot(aes(x= n, y= fct_rev(region), 
             fill = sector)) + 
    geom_col()+
    scale_x_continuous()+
...
#ggsave(filename = "figs/cluster_paper_plots/sector_per_region_plot_no.png", plot = sector_per_region_plot_no, limitsize = FALSE, width = 14, height = 7)
#ggsave(filename = "figs/cluster_paper_plots/sector_per_region_plot_se.png", plot = sector_per_region_plot_se, limitsize = FALSE, width = 14, height = 7)

### Patch together

max_value <- sector_per_region %>%
  group_by(country, region) %>%
  summarise(total_jobs = sum(n)) %>%
  ungroup() %>%
  summarise(max_jobs = max(total_jobs)) %>%
  pull(max_jobs)

combined_margins <- c(0, 0, 0, 0) # t, r, b, l

# Create plots with a shared x-axis scale
sector_per_region_plot_dk <- sector_per_region_dk %>%
  ggplot(aes(x = n, y = fct_rev(region), fill = sector)) + 
  geom_col() +
  ...
  theme(
    ...
    text = element_text(size = 14),
    plot.margin = margin(t = combined_margins[1], r = combined_margins[2],
                         b = combined_margins[3], l = combined_margins[4])
  ) +
  ggtitle("Denmark")
...
sector_per_region_plot_combined <- (sector_per_region_plot_dk / 
                 sector_per_region_plot_no / 
                 sector_per_region_plot_se) + 
                 plot_layout(guides = "collect", heights = c(1.5, 3, 1.5*18/5)) & 
                 theme(legend.position = "bottom")

# Display the combined plot
print(sector_per_region_plot_combined)

#ggsave(filename = "figs/cluster_paper_plots/sector_per_region_plot_combined.png", plot = sector_per_region_plot_combined, limitsize = FALSE, width = 20, height = 10)

```

Doing it in opposite direction - After second thoughts, this is not very nice, use previous

```{r}
region_per_sector_plot_dk <- sector_per_region_dk %>%
  ggplot(aes(x= n, y= fct_rev(sector), 
             fill = region)) + 
    geom_col()+
    scale_x_continuous()+
...
#ggsave(filename = "figs/cluster_paper_plots/region_per_sector_plot_dk.png", plot = region_per_sector_plot_dk, limitsize = FALSE, width = 14, height = 7)

region_per_sector_plot_no <- sector_per_region_no %>%
  ggplot(aes(x= n, y= fct_rev(sector), 
             fill = region)) + 
    geom_col()+
    scale_x_continuous()+
...
#ggsave(filename = "figs/cluster_paper_plots/region_per_sector_plot_no.png", plot = region_per_sector_plot_no, limitsize = FALSE, width = 14, height = 7)


region_per_sector_plot_se <- sector_per_region_se %>%
  ggplot(aes(x= n, y= fct_rev(sector), 
             fill = region)) + 
    geom_col()+
    scale_x_continuous()+
...
#ggsave(filename = "figs/cluster_paper_plots/region_per_sector_plot_se.png", plot = region_per_sector_plot_se, limitsize = FALSE, width = 14, height = 7)


```

```{r}
jobrole_per_region_plot <- jobs_clustered %>%
  mutate(place = str_replace(place, "(.*), (..)", "\\2, \\1")) %>%
  group_by(place,competence) %>%
  summarise(n =n()) %>%
  ggplot(aes(x= n, y= fct_rev(competence), 
             fill = place)) + 
    geom_col()+
    scale_x_continuous()+
    scale_fill_viridis_d("Region", option = "turbo")+
...
jobrole_per_region_plot

#ggsave(filename = "figs/cluster_paper_plots/jobrole_per_region_plot.png", plot = jobrole_per_region_plot, limitsize = FALSE, width = 15, height = 6)
...
```{r}
jobrole_per_region <- jobs_clustered %>%
  mutate(place = str_replace(place, "(.*), (..)", "\\2, \\1")) %>%
  group_by(place,competence) %>%
  summarise(n =n())%>%
  separate(place, into = c("country", "region"), sep = ", ")

jobrole_per_region_dk <- jobrole_per_region %>%
  filter(country == "DK") #%>%
#  rbind(data.frame(country = c("DK","DK"), region = c("Hovedstaden","Hovedstaden"), sector = c("Iron/steel/mining","Transport"), n = c(0,0)))
jobrole_per_region_no <- jobrole_per_region %>%
  filter(country == "NO")
jobrole_per_region_se <- jobrole_per_region %>%
  filter(country == "SE") %>%
  rbind(data.frame(country = c("SE"), region = c("Dalarna"), competence = c("Law"), n = c(0)))

### Patch together

max_value_roles <- jobrole_per_region %>%
  group_by(country, region) %>%
  summarise(total_jobs = sum(n)) %>%
  ungroup() %>%
  summarise(max_jobs = max(total_jobs)) %>%
...
# Create plots with a shared x-axis scale
jobrole_per_region_plot_dk <- jobrole_per_region_dk %>%
  ggplot(aes(x = n, y = fct_rev(region), fill = competence)) + 
  geom_col() +
  scale_x_continuous(limits = c(0, max_value)) +
  scale_fill_viridis_d("Job role", option = "turbo") +
  labs(x = "", y = "") +
  theme_minimal() +
  theme(
    legend.position = "none",
...
jobrole_per_region_plot_no <- jobrole_per_region_no %>%
  ggplot(aes(x = n, y = fct_rev(region), fill = competence)) + 
  geom_col() +
  scale_x_continuous(limits = c(0, max_value)) +
  scale_fill_viridis_d("Job role", option = "turbo") +
...
jobrole_per_region_plot_se <- jobrole_per_region_se %>%
  ggplot(aes(x = n, y = fct_rev(region), fill = competence)) + 
  geom_col() +
  scale_x_continuous(limits = c(0, max_value)) +
  scale_fill_viridis_d("Job role", option = "turbo") +
...
# Combine plots using patchwork and display a single legend
jobrole_per_region_plot_combined <- (jobrole_per_region_plot_dk / 
                 jobrole_per_region_plot_no / 
                 jobrole_per_region_plot_se) + 
                 plot_layout(guides = "collect", heights = c(1.5, 3, 1.5*18/5)) & 
                 theme(legend.position = "bottom")

# Display the combined plot
print(jobrole_per_region_plot_combined)

#ggsave(filename = "figs/cluster_paper_plots/jobrole_per_region_plot_combined.png", plot = jobrole_per_region_plot_combined, limitsize = FALSE, width = 20, height = 10)
```
...
#ggsave(filename = "figs/cluster_paper_plots/jobrole_per_region_plot_dk.png", plot = jobrole_per_region_plot_dk, limitsize = FALSE, width = 20, height = 10)
#ggsave(filename = "figs/cluster_paper_plots/jobrole_per_region_plot_no.png", plot = jobrole_per_region_plot_no, limitsize = FALSE, width = 20, height = 10)
#ggsave(filename = "figs/cluster_paper_plots/jobrole_per_region_plot_se.png", plot = jobrole_per_region_plot_se, limitsize = FALSE, width = 20, height = 10)


```

```{r}
jobrole_per_sector <- jobs_clustered %>%
  group_by(sector,competence) %>%
  summarise(n =n()) %>%
  ggplot(aes(x= n, y= fct_rev(competence), 
             fill = sector)) + 
    geom_col()+
...
#ggsave(filename = "figs/cluster_paper_plots/jobrole_per_sector.png", plot = jobrole_per_sector, limitsize = FALSE, width = 16, height = 6)

```

## Cluster plots

Using the groups on competence

```{r}
jobrole_per_cluster <- jobs_cluster_nicename %>%
  group_by(cluster,competence) %>%
  summarise(n =n()) %>%
  ggplot(aes(x= n, y= fct_rev(competence), 
             fill = cluster)) + 
    geom_col()+
...
#ggsave(filename = "figs/cluster_paper_plots/jobrole_per_cluster.png", plot = jobrole_per_cluster, limitsize = FALSE, width = 16, height = 6)
```
...
# Save table
#cluster_tbl %>% gtsave("figs/cluster_paper_plots/cluster_tbl.png")
```
...
# Create the gt table
cluster_tbl_short <- cluster_tbl_data_short %>%
  gt() %>%
  tab_header(
    title = "Regions and Sectors by Group"
  ) %>%
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = c(Group, Regions, Sectors)
    )
  )
...
# Save table
#cluster_tbl_short %>% gtsave("figs/cluster_paper_plots/cluster_tbl_short.png")
```

Remake of shorter table with highlighted regions/sectors/competences
...
Most important thing for each region - tried out some different tables to highlight most important/key/iconic factors/parts of the regions/sectors/clusters

```{r}
# most important sector/job for each region also company?
most_important_for_region <- merge(
  jobs_cluster_nicename %>%
  group_by(place, sector) %>%
  ...
  filter(n == max(n)) %>% 
  summarise(
    competence = paste(competence, collapse = ", "),
    n = first(n)
  ) %>% ungroup(),
  by = "place", all.x = TRUE
) %>%
  select(Region = place, "Most important sector" = sector, "n (sector)" = n.x,
         "Most important job role" = competence, "n (job role)" = n.y)
  

most_important_for_region_tbl <- most_important_for_region %>%
  gt() %>%
  tab_header(
    title = "Most important sector/job role by Region"
  ) %>%
  cols_label(
    Region = "Region",
    "Most important sector" = "Most important sector",
    "n (sector)" = "n (sector)",
    "Most important job role" = "Most important job role",
    "n (job role)" = "n (job role)"
  ) %>%
  tab_options(
    table.font.size = px(12),
    heading.align = "left",
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = c(Region, "Most important sector", "n (sector)", "Most important job role",
                  "n (job role)")
    )
  ) %>%
  cols_width(
    Region ~ 160,
    "Most important sector" ~ 250,
    "n (sector)" ~ 50,
    "Most important job role" ~ 250,
    "n (job role)" ~ 50
  )
  
most_important_for_region_tbl

#gtsave(
#  data = most_important_for_region_tbl,
#  filename = "figs/cluster_paper_plots/most_important_for_region_tbl.png",
#  expand = 20  # Expand margins around the image
#)
```

Most important for each cluster

```{r}
most_important_for_cluster <- merge(
  jobs_cluster_nicename %>%
  ...
  select(Group = cluster, "Most important region" = place, "n (region)" = n.x, 
         "Most important sector" = sector, "n (sector)" = n.y,
         "Most important job role" = competence, "n (job role)" = n)



most_important_for_cluster_tbl <- most_important_for_cluster %>%
  gt() %>%
  tab_header(
    title = "Most important region/sector/job role by Group"
  ) %>%
  tab_options(
    table.font.size = px(12),
    heading.align = "left",
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = c(Group, "Most important region", "n (region)", "Most important sector", 
                  "n (sector)", "Most important job role", "n (job role)")
    )
  ) #%>%
  cols_width(
    Region ~ 160,
...
most_important_for_cluster_tbl

#gtsave(
#  data = most_important_for_cluster_tbl,
#  filename = "figs/cluster_paper_plots/most_important_for_cluster_tbl.png",
#  expand = 20  # Expand margins around the image
#)
```

Most important company in region/group/sector/job role

```{r}
imprtant_company_for_country <- jobs_cluster_nicename %>%
  group_by(nation, company) %>%
  summarise(n = n()) %>% ungroup() %>%
  group_by(nation) %>%
  filter(n == max(n)) %>% 
  summarise(
...
imprtant_company_for_region <- jobs_cluster_nicename %>%
  group_by(place, company) %>%
  summarise(n = n()) %>% ungroup() %>%
  group_by(place) %>%
  filter(n == max(n)) %>% 
  summarise(
...
imprtant_company_for_cluster <- jobs_cluster_nicename %>%
...
imprtant_company_for_sector <- jobs_cluster_nicename %>%
...
imprtant_company_for_jobrole <- jobs_cluster_nicename %>%
  group_by(competence, company) %>%
  summarise(n = n()) %>% ungroup() %>%
  group_by(competence) %>%
  filter(n == max(n)) %>% 
  summarise(
...
df_hcl_nicename_long %>% 
  ...
  facet_wrap(~cluster)+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 5), # Adjust size as needed
    axis.text.y = element_text(size = 5), # Adjust size as needed
    strip.text = element_text(size = 5) # Adjust size as needed for facet labels
  ) +
  labs(x="", y ="")
...
#region competence
df_hcl_nicename_long %>% 
  ggplot(aes(x=str_trunc(place, width = 15, ellipsis = "."), 
             y=str_trunc(competence, width =15, ellipsis = "."), 
             fill = n)) +
  geom_tile() +
  scale_fill_viridis_c(direction=1, option = "D")+
...
df_hcl_nicename_long %>% 
  ggplot(aes(x=str_trunc(place, width = 15, ellipsis = "."), 
             y=str_trunc(sector, width =15, ellipsis = "."), 
             fill = n)) +
  geom_tile() +
  scale_fill_viridis_c(direction=1, option = "D")+
  facet_wrap(~cluster)+
...
Trying to create maps to visualise clusters without much success
...
mapdata <- map_data("world", region = c("Norway(?!:Svalbard)(?!:Jan Mayen)","Sweden","Denmark"))
...
combined <- rbind(sweden, norway, denmark)

map2 <- ggplot(data = combined) +
  geom_sf(aes(fill = NAME), color = "white") +
  theme_minimal() +
  labs(title = "Map of Sweden, Norway, and Denmark")
...
```{r}
numberOf_jobsPer_region <- jobs_cluster_nicename %>%
  group_by(nation, place) %>%
  summarise(n =n()) %>%
  mutate(place = str_replace(place, "(.*), (..)", "\\2, \\1"))

nrJobs_perRegion_plot <- numberOf_jobsPer_region %>%
  ggplot(aes(x= n, y= fct_rev(place),
             fill = nation)) + 
  geom_col(drop = F)+
    scale_x_continuous()+
...
#ggsave(filename = "figs/cluster_paper_plots/nrJobs_perRegion_plot.png", plot = nrJobs_perRegion_plot, limitsize = FALSE, width = 16, height = 8)
...
```{r}
(jobs_cluster_nicename %>% group_by(nation, company) %>% summarise(n =n()) %>% 
         ungroup() %>% arrange(desc(n)) %>% select(n) %>% sum())*3/4

jobs_cluster_nicename %>% group_by(nation, company) %>% summarise(n =n()) %>% 
         ungroup() %>% arrange(desc(n)) %>% head(52) %>% select(n) %>% sum()

biggest_companies <- jobs_cluster_nicename %>%
  group_by(nation, company) %>%
  summarise(n =n()) %>%
  ungroup() %>%
  arrange(desc(n)) %>%
...
biggest_companies_plot <- biggest_companies %>%
  ggplot(aes(x= n, y= fct_rev(company),
             fill = nation)) + 
  geom_col(drop = F)+
    scale_x_continuous()+
  scale_fill_colorblind("Country")+
    labs(x= "", y = "", title = "Total jobs per most recruiting companies (responsible for 67% of all jobs)")+
    theme_minimal()+
    theme(
      legend.position = "bottom", 
      text = element_text(size = 14), 
...
#ggsave(filename = "figs/cluster_paper_plots/biggest_companies_plot.png", plot = biggest_companies_plot, limitsize = FALSE, width = 16, height = 8)
```

## Table of companies in clusters

```{r}
cl_tbl_org <- df_hcl_6_comp %>% 
  select(cluster = cl, place, sector)
...
jobs_clustered <- jobs %>% # Keep in mind that cluster 2 and 3 are unclear and should be regarded as one "the rest"-cluster
  mutate(year = lubridate::year(Scrape_date), month = lubridate::month(Scrape_date)) %>%
  left_join(companies, by =c("Company" = "Name")) %>%
  select(id = ID, country = nation, region, company = Company , sector = Industry_Sector, competence = job_role, year, month, nation = country) %>%
  na.omit() %>%
  unite(place, c("region", "country"), sep = ", ") %>%
  merge(cluster_place_sector, by = c("place", "sector"), all.x = TRUE) %>%
  select(id, cluster, place, company, sector, competence, year, month, nation)
...
jobs_cluster_nicename <- jobs_clustered %>% # NOTICE the names and numbers may not work with current run
  mutate(cluster = case_when(
    cluster == 1 ~ "Norwegian Engineering",
    cluster == 2 ~ "Others",
    cluster == 3 ~ "Danish and Norwegian Research",
    cluster == 4 ~ "Others",
    cluster == 5 ~ "Central offices in Oslo/Copenhagen",
    cluster == 6 ~ "Nordic Broad Energy",
    TRUE ~ as.character(cluster)
  ))
...
cluster_tbl_sectors <- jobs_cluster_nicename %>%
  ...
  pivot_wider(names_from = "Sectors",
              values_from = Sectors) %>%
  unite("Sectors", Chemicals, "Consulting","Energy technology manufacturer","Energy Utility", "Government/Public sector", "Hydrogen technology manufacturer", "Iron/steel/mining","Legal/HR/Economics", "Oil/gas/refinery","Other", "Other manufacturing","Research/Education",   "Transport",
        sep = ", ", na.rm = TRUE)
...
cluster_tbl_jobroles <- jobs_cluster_nicename %>%
  ...
  pivot_wider(names_from = "Job_roles",
              values_from = Job_roles) %>%
  unite("Job_Roles", "Administration","Engineering", "Environmental/Sustainability",
        "Finance/Business development","HR", "IT/Data Science", "Law", "Management/CEO", "Manufacturing/Maintenance/Industrial work/Technicians", "Other", "PhD", "Procurement/Supply Chain Management/Logistics", "Project managment", "Public administration", "Quality Assurance", "Researcher", "Sales/Marketing/Customer Service",
        sep = ", ", na.rm = TRUE)
...
cluster_tbl_data_short <- merge(
  ...
  jobs_cluster_nicename %>%
    ...
    unite("Sectors", Chemicals, "Consulting","Energy technology manufacturer",
          "Energy Utility", "Government/Public sector", "Hydrogen technology manufacturer",
          "Iron/steel/mining","Legal/HR/Economics", "Oil/gas/refinery","Other", 
  ...
  merge(
    ...
    pivot_wider(names_from = "Job_roles",
                values_from = Job_roles) %>%
    unite("Job_Roles", "Administration","Engineering", "Environmental/Sustainability",
          "Finance/Business development","HR", "IT/Data Science", "Law", "Management/CEO",
          "Manufacturing/Maintenance/Industrial work/Technicians", "Other", "PhD",
          "Procurement/Supply Chain Management/Logistics", "Project managment", 
...
# Manually made more easily visible table of our old 5 clusters

```{r}
five_old_groups <- data.frame(
  Group = c("Norwegian energy engineering",
            "Danish & Norwegian research",
            "Management & finance in Oslo & Copenhagen",
            "Energy manufacturing & utilisation",
            "Hydrogen manufacturing"),
  Regions = c("Oslo (NO), Rogaland (NO), Vestland (NO), Viken (NO)", 
              "Hovedstaden (DK), Trøndelag (NO)",
              "Hovedstaden (DK), Oslo (NO)",
              "Distribution of many regions",
              "Hovedstaden (DK), Midtjylland (DK), Oslo (NO), less prominent regions"),
  Sectors = c("Consulting, Energy technology manufacturer",
              "Research & Education",
              "Consulting, Energy utility",
              "Energy technology manufacturer, Energy utility, some jobs in Fossil fuel & refinery and Hydrogen technology manufacturer",
              "Hydrogen technology manufacturer, Other manufacturer, less prominent sectors"),
  Job_roles = c("Engineering, less prominent job roles",
                "PhD, Research, som jobs in Engineering and Management",
                "Finance & Business development, Management, less prominent job roles",
                "Engineering, less prominent job roles",
                  "Engineering, Technician & Maintenance, less prominent job roles")#,
#  Number_of_jobs = c(594, 65, 152, 220, 289)
)


# Create the gt table
five_old_groups_tbl <- five_old_groups %>%
  gt() %>%
  cols_label(
    Group = "Group",
    Regions = "Regions",
...
five_old_groups_tbl %>% gtsave("figs/five_old_groups_tbl.png")
```

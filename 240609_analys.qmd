...
>>>>>>> 87ca9fac92672afa183961865ec1464ac87bec3a
```{r}
...
```{r}
engineers_norm <- tbl %>% filter(job_role == "Engineering") %>% group_by(Type, Country) %>%
  summarise(n_job = n(), .groups = 'drop') %>%
  complete(Type, Country, fill = list(n_job = 0)) 

jobs_total <- aggregate(n_job ~ Country, data = engineers_norm, sum)

engineer_total <- aggregate(n_job ~ Type, data = engineers_norm, sum)

engineers_norm <- merge(engineers_norm, jobs_total, by = "Country", suffixes = c("", "_country"))
engineers_norm <- merge(engineers_norm, engineer_total, by = "Type", suffixes = c("", "_type"))
...
engineers_deviation_plot <- ggplot(engineers_norm, aes(x = str_wrap(Type, 20, whitespace_only = FALSE), y = diff*100)) +
  geom_col(aes(fill = Country), width = 0.5, position = position_dodge(width = 0.5)) +
  scale_fill_colorblind("Country") +
  theme(legend.position = "top") +
  labs(y = "Deviation from mean", x = "") +
...
#ggsave(filename = "figs/jobroles/engineers_deviation.png", plot = engineers_deviation_plot, limitsize = FALSE, width = 10, height = 7)

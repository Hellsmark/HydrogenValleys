...
job_roles <- df_job_roles %>%
  select(ID, job_role = Job_role, Type)
...
## Residuals plot - Sectors
```{r}
sectors_tbl <- tbl %>% select(Sector,Country) %>% table()

chi_sector <- chisq.test(sectors_tbl, simulate.p.value=TRUE) 

sector_residuals <- as.data.frame(chi_sector$residuals)


sector_residuals_plot <- ggplot(sector_residuals, aes(x = str_wrap(Sector, 15, whitespace_only = FALSE), y = Freq)) +
...
sector_residuals_plot

#ggsave(filename = "figs/jobroles/sector_residuals.png", plot = sector_residuals_plot, limitsize = FALSE, width = 10, height = 7)
```
...
## Residuals plot - job roles

- Jag ska fixa till så att texten på axlarna blir bättre samt mellanrum mellan staplarna  

```{r}
jobroles_tbl <- tbl %>% select(job_role, Country) %>% table()

chi_jobroles <- chisq.test(jobroles_tbl, simulate.p.value=TRUE) 

jobroles_residuals <- as.data.frame(chi_jobroles$residuals)

jobroles_residuals_plot <- ggplot(jobroles_residuals, aes(x = str_wrap(job_role, 20, whitespace_only = FALSE), y = Freq)) +
  geom_col(aes(fill = Country), width = 0.5, position = position_dodge(width = 0.5)) +
  scale_fill_colorblind("Country") +
  theme(legend.position = "top") +
  labs(y = "Residuals", x = "") +
...
jobroles_residuals_plot

#ggsave(filename = "figs/jobroles/jobroles_residuals.png", plot = jobroles_residuals_plot, limitsize = FALSE, width = 10, height = 7)
```
...
gt_tbl <- tbl %>% filter(job_role == "Engineering") %>%
  tbl_cross(Type, Country, percent = "cell") %>% #could also change to Type_2, for a different categorisation
  as_gt() %>%
  gt_theme_espn() 
...
tbl %>% filter(job_role == "Engineering") %>%
  ...
  summarise(n = n()) %>%
  left_join(engineers_n, by = "Type") %>%
  ggplot(aes(x = n, y = fct_reorder(Type, total_n), fill = Country)) + 
  geom_col()+
    scale_x_continuous()+
    scale_fill_colorblind("Country")+
...
#ggsave("figs/Total_jobs_per_engineering_type.png", dpi = 1200, width = 6, height = 8)

```
### Residuals plot - engineers

```{r}
engineers_tbl <- tbl %>% filter(job_role == "Engineering") %>% select(Type, Country) %>% table()

chi_engineers <- chisq.test(engineers_tbl, simulate.p.value=TRUE) 

engineers_residuals <- as.data.frame(chi_engineers$residuals)


engineers_residuals_plot <- ggplot(engineers_residuals, aes(x = Type, y = Freq)) +
  geom_col(aes(fill = Country), width = 0.5, position = position_dodge(width = 0.5)) +
  scale_fill_colorblind("Country") +
  theme(legend.position = "top") +
  labs(y = "Residuals", x = "") +
  theme_bw() +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1, vjust = 1)) 
engineers_residuals_plot

#ggsave(filename = "figs/jobroles/engineers_residuals.png", plot = engineers_residuals_plot)
```
